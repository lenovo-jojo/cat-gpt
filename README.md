...
## 🗨️ "Feeling chatty" In-Game Option

- After a villager delivers two unique lines in a conversation, the mod replaces the first entry in the next choice menu with **Feeling chatty**.
- Choosing **Feeling chatty** triggers the screenshot capture + LLM pipeline so the follow-up dialogue is generated by the model.
- The rewritten menu item keeps the original jump target, so gameplay flow continues exactly as if you had selected the first option.
- Tests covering this behavior live in `tests/test_feeling_chatty.py`; run `python -m pytest tests/test_feeling_chatty.py` to confirm the injection works.
...
Seeing only an “Add files via upload” commit on GitHub usually means no new commits have been pushed yet. Follow these steps any time you replace a file locally and want the diff to appear on GitHub:

1. **Check Git status** to confirm Git sees your changes:
   ```bash
   git status
   ```
2. **Stage the files** you updated (for example, after pasting a fresh `ac_parser_encoder.py`):
   ```bash
   git add ac_parser_encoder.py
   ```
3. **Create a commit** with a descriptive message:
   ```bash
   git commit -m "Update parser encoder with Feeling chatty option"
   ```
4. **Push to GitHub** on the branch you are viewing in the browser:
   ```bash
   git push origin <your-branch-name>
   ```
5. **Refresh the GitHub page** (or open a pull request). The new commit now shows the diff so you can review or share it.

Tip: repeat the same workflow for every batch of changes—GitHub renders diffs from commits, so pushing each commit keeps the web UI in sync with your local edits.
